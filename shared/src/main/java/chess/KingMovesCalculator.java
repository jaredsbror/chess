...
public class KingMovesCalculator implements PieceMovesCalculator {
    ...
    public Collection<ChessMove> pieceMoves( ChessBoard board, ChessPosition position ) {
        ...
        // Optional debug
        if ( Constants.DEBUG_GLOBAL || Constants.DEBUG_KING_MOVES_CALCULATOR )
            System.out.println( "(KingCalculator) Found moves from (" + position.getRow() + "," + position.getColumn() + ") for " + board.getPiece( position ).toString() + ":-> " + validMoves.toString() );
        return validMoves;
    }
